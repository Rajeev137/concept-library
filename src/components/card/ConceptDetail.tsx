  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 bg-[var(--bg-primary)] rounded-lg shadow-xl p-6 max-w-sm w-full mx-4 border border-[var(--border-default)]">
        <h2 id="delete-dialog-title" className="text-base font-semibold text-[var(--text-primary)] mb-2">
          Delete this concept?
        </h2>
  ...
  if (error?.status === 404 || error?.message === "Not found") {
    return (
      <div className="p-6 text-center">
        <p className="text-[var(--text-muted)] text-sm">Concept not found</p>
      </div>
    );
  }
    ...
    <>
      ...
      <article className="p-6 max-w-2xl space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button
              ...
              className="px-3 py-1.5 text-sm rounded-md border border-[var(--danger-border)] text-[var(--danger)] hover:bg-[var(--bg-tertiary)] transition-colors"
            >
              Delete
            </button>
          </div>
        ...
        <section aria-labelledby="what-it-does-label">
          <h2 id="what-it-does-label" className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-1">
            What it does
          </h2>
          <p className="text-sm text-[var(--text-secondary)] whitespace-pre-wrap">{concept.what_it_does}</p>
        </section>

        {/* Comparisons */}
        {sortedComparisons.length > 0 && (
          <section aria-labelledby="comparisons-label">
            <h2 id="comparisons-label" className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-2">
              Comparisons
            </h2>
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-[var(--border-default)]">
                  <th className="text-left py-1.5 pr-4 font-medium text-[var(--text-muted)] w-1/3">
                    Alternative
                  </th>
                  <th className="text-left py-1.5 font-medium text-[var(--text-muted)]">
                    Difference
                  </th>
                </tr>
              </thead>
              <tbody>
                {sortedComparisons.map((c) => (
                  <tr key={c.id} className="border-b border-[var(--border-subtle)] last:border-0">
                    <td className="py-2 pr-4 font-medium text-[var(--text-primary)] align-top">
                      {c.alternative}
                    </td>
                    <td className="py-2 text-[var(--text-secondary)] align-top">
                      {c.difference}
                    </td>
                  </tr>
                ))}
        ...
        <section aria-labelledby="when-it-breaks-label">
          <h2 id="when-it-breaks-label" className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-1">
            When it breaks
          </h2>
          <p className="text-sm text-[var(--text-secondary)] whitespace-pre-wrap">{concept.when_it_breaks}</p>
        </section>

        {/* Explain in 30s */}
        <section aria-labelledby="explain-label">
          <h2 id="explain-label" className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-1">
            Explain in 30s
          </h2>
          <blockquote className="border-l-[3px] border-[var(--quote-border)] bg-[var(--quote-bg)] pl-4 pr-3 py-3 rounded-r-md text-sm text-[var(--text-secondary)] italic whitespace-pre-wrap">
        ...
        <section aria-labelledby="where-used-label">
          <h2 id="where-used-label" className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-1">
            Where I used it
          </h2>
          <p className="text-sm text-[var(--text-secondary)] whitespace-pre-wrap">{concept.where_i_used_it}</p>
        </section>

        {/* Tags */}
        {concept.tags.length > 0 && (
          <section aria-labelledby="tags-label">
            <h2 id="tags-label" className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-2">
              Tags
            </h2>
              ...
              {concept.tags.map((tag) => (
                <li key={tag}>
                  <button
                    ...
                    className="px-2 py-0.5 rounded-full bg-[var(--tag-bg)] text-xs text-[var(--tag-text)] border border-[var(--border-default)] hover:opacity-80 transition-colors"
                  >
                    {tag}
                  </button>
                </li>
        ...
        {concept.image && (
          <section aria-labelledby="image-label">
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] hover:text-[var(--text-secondary)] transition-colors mb-2"
            >
              <span
            ...
            {imageExpanded && (
              <div className="relative w-full aspect-video">
                <Image
                  ...
                  fill
                  className="rounded-md border border-[var(--border-default)] object-contain"
                />
              </div>
            )}
          </section>
